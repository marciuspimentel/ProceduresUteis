...
Begin  
   ...
   If(@ColumnId = 1)  
	Print '    ' + Master.dbo.PadRight(@ColumnName,@PadRight,' ') + ' = ' + @ColumnParameter
   Else 
	Print '   ,' + Master.dbo.PadRight(@ColumnName,@PadRight,' ') + ' = ' + @ColumnParameter
     Select @ColumnId = @ColumnId + 1  
End  
...
Begin  	
	...
	If(@TableId = 1)
		Print Master.dbo.PadRight('From dbo.'+@TableName,@PadRight,' ') + ' ' + @PrefixFrom + '(Nolock) '
	Else
		Print Master.dbo.PadRight('Inner Join dbo.'+@TableName,@PadRight,' ') + ' ' + @PrefixTo + '(Nolock) On ' + @ColumnFrom + ' = ' + @ColumnTo
...
Begin  
    ...
    If(@ColumnId = 1)
		Print 'Where  (' + Master.dbo.PadRight(@ColumnName,@PadRight,' ') + ' = @' + @ColumnParameter + ' Or @' + @ColumnParameter + ' = 0)'
	Else 
		Print '   And (' + Master.dbo.PadRight(@ColumnName,@PadRight,' ') + ' = @' + @ColumnParameter + ' OR @' + @ColumnParameter + Case When(@TypeName IN ('Int', 'Decimal', 'Money','Numeric')) Then '= 0)' Else  ' IS NULL)' End  
    Select @ColumnId = @ColumnId + 1  
End    
Print ''  
Print 'End'  
